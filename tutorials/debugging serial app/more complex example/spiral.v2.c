  ...
  for ( int i = 0; i < Nregions; i++ )
    {
      ...
      if(i & 1) // index is odd -> subregion is on the right
	{
	  offset = N_2 - Regions[0][BL][x_]; // offset the insertion on the right by the left part
	  start  = offset;
	}
      else
	offset = Regions[1][TR][x_] - N_2;
